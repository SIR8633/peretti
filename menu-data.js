...
// Versión del menú para invalidación automática de caché en navegadores móviles
const PERETTI_DATA_VERSION = "2026-10-01-v2";

function checkAndMigrateCache() {
    try {
        if (typeof localStorage === "undefined") return;
        const savedVersion = localStorage.getItem("peretti_data_version");
        if (savedVersion !== PERETTI_DATA_VERSION) {
            console.log(`[Peretti] Actualizando versión de datos: ${savedVersion || 'ninguna'} -> ${PERETTI_DATA_VERSION}`);
        ...
        console.warn("[Peretti] No se pudo acceder a localStorage", e);
    }
}
...
function getActiveBusinessConfig() {
    checkAndMigrateCache();
    if (typeof localStorage === "undefined") return defaultBusinessConfig;
...
function getActiveMenuData() {
    checkAndMigrateCache();
    if (typeof localStorage === "undefined") return defaultMenuData;
...
function getActiveExtras() {
    checkAndMigrateCache();
    if (typeof localStorage === "undefined") return defaultExtras;
